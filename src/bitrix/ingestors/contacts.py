...
SELECT_FIELDS = [
    "ID","NAME","LAST_NAME","SECOND_NAME","TYPE_ID","COMPANY_ID","DATE_CREATE","DATE_MODIFY",
    "ASSIGNED_BY_ID","PHONE","EMAIL"
]

def full_sync(client: BitrixClient, limit: Optional[int] = None) -> int:
    count = 0
    for item in client.list_paginated("crm.contact.list", select=SELECT_FIELDS, order={"ID": "ASC"}, include_total=True):
        upsert_entity("contacts", item)
        count += 1
...
def incremental_sync(client: BitrixClient, since: Optional[datetime] = None, limit: Optional[int] = None) -> int:
    """
    Sync only contacts that were created or modified since last sync.
    ...
    logger.info(f"Incremental sync for contacts created or modified since {since_str}")
    ...
    logger.info(f"Incremental sync completed: {count} contacts")
    return count
    ...
    try:
        ...
        with eng.connect() as conn:
            result = conn.execute(text(
                "SELECT last_sync_at FROM bitrix.sync_state WHERE entity = 'contacts'"
            )).fetchone()
        ...
        with eng.begin() as conn:
            if is_full:
                conn.execute(text("""
                    ...
                    VALUES ('contacts', now(), now(), :count, 'completed')
                ...
                conn.execute(text("""
                    ...
                    VALUES ('contacts', now(), :count, 'completed')
                    ON CONFLICT (entity) DO UPDATE SET
                        last_sync_at = now(),

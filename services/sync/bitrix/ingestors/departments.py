...
def full_sync(client: BitrixClient, limit: Optional[int] = None) -> int:
    """Sync all departments from Bitrix24"""
    count = 0
    
    # Department.get tüm departmanları döndürür
    response = client.call("department.get", {})
    ...
    if not result:
        logger.warning("No departments in result")
        return 0
    ...
    for item in result:
        ...
        upsert_entity("departments", item)
        count += 1
    ...
    logger.info(f"Full sync completed: {count} departments")
    return count
...
def _get_last_sync() -> Optional[datetime]:
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text("SELECT last_sync_at FROM bitrix.sync_state WHERE entity = 'departments'")
        )
        row = result.fetchone()
        ...
        if is_full:
            conn.execute(
                text("""
                    ...
                    VALUES ('departments', :now, :now, :count, 'completed', :now)
            ...
            conn.execute(
                text("""
                    ...
                    VALUES ('departments', :now, :count, 'completed', :now)
                    ON CONFLICT (entity) DO UPDATE SET
                        last_sync_at = :now,
